...
**Live demo:** [Sand and Sky](https://rohitjz.github.io/claude-for-pms/) (default, warm paper) · [Dark](https://rohitjz.github.io/claude-for-pms/dark.html) (Linear-inspired). Both pages are generated by `glassify`, one of the skills in this repo.
...
### 3. Ship an Apple-grade landing page in 5 minutes (`glassify`)

You say: *"Landing page for my AI meeting notetaker. Audience: sales teams. CTA: book a demo."*

The skill asks four more questions (theme, vibe in three words, sections you want, hero pitch) and produces a single `index.html` with:

- Animated monochromatic mesh background tuned to your theme (Sand and Sky / Linear dark / Warm Authority / custom)
- Floating glass nav at the top, SVG displacement refraction so content behind distorts as you scroll (not blurs)
- 8-layer dome shadow on every glass panel: top specular highlight, side refraction hints, bottom subtle shadow. Reads as 3D crystal, not frosted fog.
- Solid-accent CTAs with inset-highlight borders, spring-eased hover
- Scroll reveal on every section, cubic-bezier overshoot easing
- Zero build step, zero runtime dependencies (Inter font is the only CDN), under 45KB
...
**What `glassify` gives you (hero section, abbreviated):**
...
Wrapped in a full design system: tokens for surfaces, text, borders, blur, dome shadow, motion. Four theme presets (Sand and Sky, Linear dark, Warm Authority, custom). Three animated background recipes (monochromatic mesh, floating orbs, conic aurora). Open in browser, screenshot for the pitch deck, ship.
...
cp -r claude-for-pms/{patrick-winston-presentation,council,glassify} ~/.claude/skills/
```
